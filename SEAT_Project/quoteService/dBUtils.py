...
class DBUtils (object):
    ...
    def lookUpPricesInDb(self, lido_id):
        """Gets beach club's prices in dynamoDB

        Args:
            lido_id (string): beach club's username

        Returns:
            dict: Dictionary that contains all beach club's prices 
        """
        
        dynamoDbTable = self.dynamoDb.Table('pricePerPiece')
        response = dynamoDbTable.scan(FilterExpression=Attr('lidoId').eq(lido_id))
        data = response['Items']
        
        while 'LastEvaluatedKey' in response:
            response = self.utenti.scan(ExclusiveStartKey=response['LastEvaluatedKey'])
            data.extend(response['Items'])   
        
        prezzi = {}        
        for item in data:
            prezzi.update({item['pezzo']:int(item['costo'])})
        return prezzi
    ...
    def scanDb(self, table, filterAttributeList, filterValueList):
        ...
        try:
            ...
            if len(filterAttributeList) == 0 and len(filterValueList) == 0:
